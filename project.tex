...
\item Effects of channel on the communication system:
  ...
  \item Definitions of $\text{SNR}_{bit}^{(DD)}$ and $\text{SNR}_{sym}^{(DD)}$: 
    \begin{eqnarray*}
      \text{SNR}_{sym}^{(DD)} &=& \frac{E_s}{N_0} = \frac{P_rE_g}{N_0}\sum_{k=0}^{M-1}{p_k|A_k|^2 } \\
...
\item Because we are using code gray, the following relation holds whenever the SNR is high enough to make transitions to far symbols unlikely:
    ...
    \end{lstlisting}
    For convenience we also find the relation between the noise variance and $\gamma_d$:
    \begin{equation} 
      \sigma_z^2 = \frac{N_0^2}{2P_r} \gamma_d
    \end{equation}
...
\begin{itemize}
\item $p_{err}=10^{-3} \Rightarrow \gamma_{d_{max}} = 10.8276$
\item $p_{err}=0.2 \Rightarrow \gamma_{d_{max}} = 1.6424$
\end{itemize}
